...
function assertSnapshot(workflow) {
  if (![2, 3, 4].includes(workflow.schema_version)) throw new Error(`workflow ${workflow.name || "?"}: unsupported schema_version`);
...
export function buildInstructions(workflow, outDir) {
  ...
  const lines = [
    ...
    `uses: ${JSON.stringify(playbook ? [`../../../playbooks/${playbook}`] : [])}`,
  ...
  lines.push(
    ...
    "",
    `_Generated by amf-dirf. schema_version ${workflow.schema_version || 1}. Rebuild locally to refresh routing and capabilities._`,
  );
